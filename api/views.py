...
# def get_available_slots(request, username, date_str):
#     """
#     Returns available 1-hour slots.
#     Fixes: Filters out past times for 'today' and handles Timezones correctly.
#     """
#     # 1. Validate Doctor
#     try:
#         doctor = User.objects.get(username=username, role='doctor')
...
#         today = timezone.localtime().date() # Use server local date
...
#     # 3. Get Schedule
#     # Python: 0=Mon, 6=Sun. Your Model: 1=Mon, 7=Sun.
...
#     # 4. Get Booked Slots (Timezone Aware)
#     existing_appointments = Appointment.objects.filter(
#         doctor=doctor,
#         appointment_time__date=query_date,
#         status='Upcoming'
#     )
    
#     # Convert DB UTC times to Local Time for comparison
#     booked_times = set()
#     for apt in existing_appointments:
#         # Convert UTC -> Local Time before extracting .time()
#         local_dt = timezone.localtime(apt.appointment_time)
...
#     # Get current local time to filter out past slots for today
#     now_local = timezone.localtime()

#     while current_time < end_time:
#         slot_time_obj = current_time.time()

#         # CHECK 1: Is this slot already booked?
#         if slot_time_obj not in booked_times:
            
#             # CHECK 2: If date is today, is the time in the past?
#             is_past_time = False
#             if query_date == today:
#                 # Compare full datetime to handle hours/minutes correctly
#                 if current_time <= now_local.replace(tzinfo=None): 
#                     is_past_time = True
            
#             if not is_past_time:
#                 # Add to list formatted nicely
#                 available_slots.append(current_time.strftime('%I:%M %p')) # "09:00 AM"
...
from rest_framework.decorators import api_view, permission_classes
from rest_framework.permissions import IsAuthenticated, AllowAny
from rest_framework.exceptions import ValidationError, PermissionDenied # Import PermissionDenied here
from rest_framework_simplejwt.views import TokenObtainPairView
...
from .serializers import (
    DoctorProfileSerializer, StaffProfileSerializer, StudentProfileSerializer,
    CaretakerProfileSerializer, UserSerializer, MyTokenObtainPairSerializer, 
    SOSAlertSerializer, AppointmentCreateSerializer, AppointmentListSerializer,
    PrescriptionDetailSerializer, PrescriptionCreateSerializer, 
    StaffStudentVitalsSerializer, DoctorListSerializer, DoctorScheduleSerializer
)
        ...
        try:
            ...
            try:
                ...
                if student_profile.hostel and student_profile.hostel.caretaker:
                    ...
                    if caretaker_user.role == 'caretaker': 
                        caretaker_group_name = f"caretaker_{caretaker_user.id}"
                        student_id_to_call = self.request.user.username
...
class AvailableDoctorListView(generics.ListAPIView):
    """
    Lists doctors available on a specific date passed via query param.
    Usage: GET /api/doctors/list-available/?date=2025-11-05
    """
    serializer_class = DoctorProfileSerializer
    permission_classes = [permissions.IsAuthenticated] 

    def get_queryset(self):
        # 1. Get the date from the URL (e.g., ?date=2025-11-05)
        date_str = self.request.query_params.get('date')
        
        if date_str:
            try:
                # Use the date sent from the phone
                target_date = datetime.strptime(date_str, "%Y-%m-%d").date()
                
                # Python weekday: 0=Monday, 6=Sunday
                # Your DB Model: 1=Monday, 7=Sunday
                # We add +1 to match your DB format
                target_day = target_date.weekday() + 1
            except ValueError:
                # Fallback to server time if format is wrong
                target_day = timezone.now().weekday() + 1
        else:
            # Fallback to server time if no date provided
            target_day = timezone.now().weekday() + 1
        
        # 2. Find doctors scheduled for that specific day
        scheduled_doctor_ids = DoctorSchedule.objects.filter(
