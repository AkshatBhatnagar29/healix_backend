...
class SOSConsumer(AsyncWebsocketConsumer):
    """
    Handles WebSocket connections for doctors to receive global SOS alerts.
    """
    async def connect(self):
        user = self.scope["user"]
        print(f"--- [SOSConsumer] Connection Attempt by {user} ---")

        if user.is_anonymous or getattr(user, 'role', '') != 'doctor':
            print("❌ [SOSConsumer] Rejected: Not a doctor or not authenticated.")
    ...
    async def disconnect(self, close_code):
        ...
        print(f"👋 [SOSConsumer] Disconnected for {self.scope['user']}")

    async def sos_notification(self, event):
        await self.send(text_data=json.dumps({
            'type': 'sos_notification',
            'sos_event_id': event['message'].get('sos_event_id')
        }))
...
class CaretakerConsumer(AsyncWebsocketConsumer):
    """
    Handles WebSocket connections for individual caretakers to receive hostel-specific alerts.
    """
    async def connect(self):
        user = self.scope["user"]
        print(f"--- [CaretakerConsumer] Connection Attempt by {user} ---")

        if user.is_anonymous or getattr(user, 'role', '') != 'caretaker':
            print("❌ [CaretakerConsumer] Rejected: Not a caretaker or not authenticated.")
            await self.close(code=4001)
            return
        ...
        await self.channel_layer.group_add(self.group_name, self.channel_name)
        await self.accept()
        print(f"✅ [CaretakerConsumer] Connected: {user.username} in {self.group_name}")
    ...
    async def receive(self, text_data):
        """
        Placeholder for handling caretaker-side WebSocket messages (e.g., accepting calls).
        """
        data = json.loads(text_data)
        print(f"[CaretakerConsumer] Received data: {data}")
        # handle future actions here (e.g., call acknowledgments)
...
class CallConsumer(AsyncWebsocketConsumer):
    """
    Handles peer-to-peer WebRTC signaling between authenticated users.
    """
    async def connect(self):
        user = self.scope["user"]
        print(f"--- [CallConsumer] Connection Attempt by {user} ---")

        if user.is_anonymous:
            print("❌ [CallConsumer] Rejected: Not authenticated.")
            await self.close(code=4001)
            return
    ...
    async def disconnect(self, close_code):
        if hasattr(self, 'group_name'):
            await self.channel_layer.group_discard(self.group_name, self.channel_name)
        print(f"👋 [CallConsumer] Disconnected for {self.scope['user']}")
    ...
    async def receive(self, text_data):
        ...
        if not target_user_id:
            print("⚠️ [CallConsumer] Received message without a target.")
            return
